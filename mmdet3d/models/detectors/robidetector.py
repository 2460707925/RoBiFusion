...
class BiRobustDetector(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        super(BiRobustDetector,self).__init__()
        # img_branch
        if img_backbone:
    ...
    def forward_test(self, points=None, img_metas=None, img=None, **kwargs):
        ...
        # print(f'extract_feats time:{t2-t1},extract_bev_feats time:{t3-t2},bev_bbox_head time:{t4-t3},bbox_postprocess time:{t5-t4}')
        # print(f'extract_feats time:{(t2-t1)/(total_time)},extract_bev_feats time:{(t3-t2)/(total_time)},bev_bbox_head time:{(t4-t3)/(total_time)},bbox_postprocess time:{(t5-t4)/(total_time)}')
    ...
    def extract_bev_feats(self,pts_feats,depth_feats,context_feats,pts_coors,img_metas):
        ...
        if self.fuse_bev_encoder is not None:
            fused_bev_feats=self.fuse_bev_encoder(pts_bev_feats,img_bev_feats)
        else:
            fused_bev_feats=img_bev_feats if img_bev_feats is not None else pts_bev_feats        
    ...
    def extract_feats(self, points, img, img_metas):
        ...
        # total_time=t4-t1
        # print(f'img_backbone time:{t1_2-t1_1},depth_net time:{t1_3-t1_2}')
        # print(f'img_backbone time:{(t1_2-t1_1)/(total_time)},depth_net time:{(t1_3-t1_2)/(total_time)}')
        # print(f'extract_img_feats time:{t2-t1},extract_pts_feats time:{t3-t2},extract_bi_feats time:{t4-t3}')
        # print(f'extract_img_feats time:{(t2-t1)/(total_time)},extract_pts_feats time:{(t3-t2)/(total_time)},extract_bi_feats time:{(t4-t3)/(total_time)}')
    ...
    def extract_img_feats(self, img, img_metas):
        ...
        # print(f'img_backbone time:{t2-t1},img_neck time:{t4-t3}')      
        return img_feats
    ...
    def extract_bi_feats(self,img_feats,pts_feats,depth_feats,pts_coors,pts_centroids,img_metas):
        ...
        # print(f"bi_fusion_layer time:{t2-t1}")
    ...
    def init_weights(self, pretrained=None):
        ...
        if pretrained is not None:
            assert isinstance(pretrained,dict),f'pretrained should be a dict, got {type(pretrained)}'            
            pretrained_keys=pretrained.keys()
            
            if self.img_backbone is not None:
                if 'img_backbone' in pretrained_keys:
                    self.load_pretrained_weights('img_backbone',pretrained['img_backbone'])
                else:
                    self.img_backbone.init_weights()
            
            if self.img_neck is not None:
                if 'img_neck' in pretrained_keys:
                    self.load_pretrained_weights('img_neck',pretrained['img_neck'])
                    # self.img_neck.init_weights(pretrained=pretrained['img_neck'])
                else:
                    self.img_neck.init_weights()
            
            if self.img_bev_encoder is not None:
                if 'img_bev_encoder'  in pretrained_keys:
                    self.load_pretrained_weights('img_bev_encoder',pretrained['img_bev_encoder'])
                    # self.img_bev_encoder.init_weights(pretrained=pretrained['img_bev_encoder'])
                # else:
                #     self.img_bev_encoder.init_weights()
                    
            if self.depth_net is not None:
                if 'depth_net' in pretrained_keys:
                    self.load_pretrained_weights('depth_net',pretrained['depth_net'])
                    # self.depth_net.init_weights(pretrained=pretrained['depth_net'])
                # else:
                #     self.depth_net.init_weights()

            if self.pts_voxel_encoder is not None:
                if 'pts_voxel_encoder' in pretrained_keys:
                    self.load_pretrained_weights('pts_voxel_encoder',pretrained['pts_voxel_encoder'])
                    # self.pts_voxel_encoder.init_weights(pretrained=pretrained['pts_voxel_encoder'])
                # else:
                #     self.pts_voxel_encoder.init_weights()
                    
            if self.pts_middle_encoder is not None:
                if 'pts_middle_encoder' in pretrained_keys:
                    self.load_pretrained_weights('pts_middle_encoder',pretrained['pts_middle_encoder'])
                    # self.pts_middle_encoder.init_weights(pretrained=pretrained['pts_middle_encoder'])
                # else:
                #     self.pts_middle_encoder.init_weights()
            
            if self.bi_fusion_layer:
                if 'bi_fusion_layer' in pretrained_keys:
                    self.load_pretrained_weights('bi_fusion_layer',pretrained['bi_fusion_layer'])
                    # self.bi_fusion_layer.init_weights(pretrained=pretrained['bi_fusion_layer'])
                # else:
                #     self.bi_fusion_layer.init_weights()
            
            if self.fuse_bev_encoder:
                if 'fuse_bev_encoder' in pretrained_keys:
                    self.load_pretrained_weights('fuse_bev_encoder',pretrained['fuse_bev_encoder']) 
                    # self.fuse_bev_encoder.init_weights(pretrained=pretrained['fuse_bev_encoder'])
                # else:
                #     self.fuse_bev_encoder.init_weights()
            
            if self.bev_backbone:
                if 'bev_backbone' in pretrained_keys:
                    self.load_pretrained_weights('bev_backbone',pretrained['bev_backbone'])
                    # self.bev_backbone.init_weights(pretrained=pretrained['bev_backbone'])
                else:
                    self.bev_backbone.init_weights()
            
            if self.bev_neck:
                if 'bev_neck' in pretrained_keys:
                    self.load_pretrained_weights('bev_neck',pretrained['bev_neck'])
                    # self.bev_neck.init_weights(pretrained=pretrained['bev_neck'])
                else:
                    self.bev_neck.init_weights()
                
            if self.bev_bbox_head:
                if 'bev_bbox_head' in pretrained_keys:
                    self.load_pretrained_weights('bev_bbox_head',pretrained['bev_bbox_head'])
                    # self.bev_bbox_head.init_weights(pretrained=pretrained['bev_bbox_head'])
                else:
                    self.bev_bbox_head.init_weights() 
        else:
            if self.img_backbone is not None:
                self.img_backbone.init_weights()
        ...
        if self.freeze_module is not None:
            assert isinstance(self.freeze_module,list),f'freeze_module should be a list, got {type(self.freeze_module)}'
            for key in self.freeze_module:
                for param in getattr(self,key).parameters():
                    param.requires_grad=False
                    
                    
    def load_pretrained_weights(self,module_name,weights_path):
        weights = torch.load(weights_path)
        getattr(self,module_name).load_state_dict(weights)
