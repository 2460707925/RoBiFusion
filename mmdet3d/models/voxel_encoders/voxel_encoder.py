...
@VOXEL_ENCODERS.register_module()
class MyDynamicSimpleVFE(nn.Module):
    """Simple dynamic voxel feature encoder used in DV-SECOND.
    ...
    def __init__(self,
                 voxel_size=(0.2, 0.2, 4),
                 point_cloud_range=(0, -40, -3, 70.4, 40, 1)):
        super(MyDynamicSimpleVFE, self).__init__()
        self.scatter = DynamicScatter(voxel_size, point_cloud_range, True)
        self.fp16_enabled = False
    ...
    def forward(self, features, coors):
        ...
        # num_points: [concated_num_points]
        features, features_coors = self.scatter(features, coors)
        return features, features_coors, None
    
    
@VOXEL_ENCODERS.register_module()
class MyDynamicVFE(nn.Module):
    """Dynamic Voxel feature encoder used in DV-SECOND.
    ...
    def __init__(self,
        ...
        super(MyDynamicVFE, self).__init__()
        assert mode in ['avg', 'max']
        assert len(feat_channels) > 0
        if with_cluster_center:
            in_channels += 3
    ...
    def forward(self,
        ...
        if self.return_centroids:
            return voxel_feats, voxel_coors, voxel_mean
        else:
            return voxel_feats, voxel_coors, None
