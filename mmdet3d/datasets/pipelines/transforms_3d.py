...
#         transform[:3, :3] = input_dict["pcd_rotation"] * input_dict["pcd_scale_factor"]
#         transform[:3, 3] = input_dict["pcd_trans"]
...
class GlobalRotScaleTrans:
    ...
    def __call__(self, data):
        ...
        if self.is_train:
            ...
            translation = np.array([random.normal(0, self.trans_lim[i]) for i in range(3)])
            ...
            rot_mat_T = torch.tensor([[rot_cos, -rot_sin, 0],[rot_sin, rot_cos, 0], [0, 0, 1]])
            transform[:3, :3] = rot_mat_T * scale
