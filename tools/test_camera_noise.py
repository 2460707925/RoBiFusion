...
def forward_test(self, points=None, img_metas=None, img=None, **kwargs):
    ...
    global depth_refine_module,saved_bev_feats_src,saved_bev_feats_tgt
    img=add_camera_noise(img,noise_cfg={'noise':'blockout'})
...
def generate_gt_depth_feat(gt_depths, depth_module):
    ...
    depth_feature=generate_depth_feature(output_shape,depth_labels).view(B*N,h,w,-1).permute(0,3,1,2)
...
def add_camera_noise(img,noise_cfg):
    noise=noise_cfg['noise']
    if noise=='gaussian':
        mean = noise_cfg.get('mean', 0.)
        std = noise_cfg.get('std', 1.)
        noise = torch.randn_like(img) * std + mean
        img=img+noise
    elif noise=="salt and pepper":
        prob = noise_cfg.get('prob', 0.01)  # 椒盐噪声的概率
        mask = torch.rand_like(img) < prob  # 创建一个随机掩码
        img[mask] = 0  # 将掩码中的像素设置为0或255
    elif noise=="blockout":
        block_size = noise_cfg.get('block_size', 30)  # 缺失像素块的大小
        num_blocks = noise_cfg.get('num_blocks', 5) 
        h, w = img.shape[-2:]
        for _ in range(num_blocks):
            x = torch.randint(0, w, (1,)).item()
            y = torch.randint(0, h, (1,)).item()
            img[..., y:y+block_size, x:x+block_size] = 0  # 将像素块设置为0
    elif noise=="affine":
        degrees = noise_cfg.get('degrees', 5)  # 旋转的角度范围
        # translate = noise_cfg.get('translate', (0.1, 0.1))  # 平移的范围
        transform = RandomAffine(degrees)
        b,n,c,h,w=img.shape
        img = transform(img.view(b*n,c,h,w)).view(b,n,c,h,w)
    else:
        pass
    return img 
